...
const SendPanel = ({ onNavigate }) => {
  ...
  const [transferStatus, setTransferStatus] = useState(null); // null | 'success' | 'error'
  ...
  const handlePickDirectory = async () => {
    ...
    try {
      // handle both sync return and Promise-based electronAPI
  ...
  const handleTransfer = async () => {
    if (!selectedIp || !directory) return;
    ...
    try {
      const res = await new Promise((resolve, reject) => {
        try {
          const result = addon.runTransfer("send", selectedIp, DEFAULT_PORT, directory);
          // sender only knows: target IP, port, and which directory to send
          resolve(result);
      ...
      <main style={styles.main}>
        ...
        <div style={styles.content}>
          ...
          <section style={{...styles.section, ...(step === "scan" ? styles.sectionLocked : {})}}>
            ...
            <p style={styles.sectionDesc}>
              Choose your target IP and the directory you wish to transmit.
            </p>
            {step !== "scan" && (
              <>
                ...
                <div style={styles.fieldGroup}>
                  <label style={styles.label}>SOURCE DIRECTORY</label>
                  <div style={styles.dirRow}>
                    <div style={styles.dirDisplay}>
                      {directory || <span style={styles.placeholder}>No directory selected</span>}
                    </div>
          ...
          <section style={{...styles.section, ...(step !== "select" && step !== "transfer" ? styles.sectionLocked : {})}}>
            ...
            <p style={styles.sectionDesc}>
              Confirm target and directory, then initiate the transfer sequence.
            </p>
              ...
              <>
                ...
                <div style={styles.actionRow}>
                  <button
                    style={{
                      ...styles.btnPrimary,
                      ...((isTransferring || !selectedIp || !directory) ? styles.btnDisabled : {})
                    }}
                    onClick={handleTransfer}
                    disabled={isTransferring || !selectedIp || !directory}
                  >
